# Capability: Thread-Safe Image Cache

## Purpose

Defines actor-isolated image caching for the app’s two-tier cache (memory → disk) used by `CachedAsyncImageView` and injected via `DIContainer`. Ensures safe concurrent access under Swift strict concurrency checking without changing user-visible caching behaviour.
