...
- Include docs/domain-data-standards.md for Domain and Data layer work (Clean Architecture, Swift, async/await, XCTest, repositories, use cases).
...
- Set the tech stack, architecture, and domain context for this specific project (iOS, Swift, SwiftUI, Clean Architecture, Rick and Morty API).
```
...
context: |
  ...
  Domain: iOS app consuming the Rick and Morty REST API (rickandmortyapi.com)
    — Characters, Episodes, Locations with pagination and image caching
...
- `docs/api-spec.yml` and `docs/data-model.md` do not exist yet in this project.
  Create them when you want to document the Rick & Morty API endpoints consumed
  by the app and the domain entities (`CharacterEntity`, `EpisodeEntity`, `LocationEntity`).
  Once created, add them to the `context` block above.
- The `ai-specs/skills/` directory contains reusable workflow skills
  (`enrich-us`, `commit`, `adversarial-review`, `code-auditing`, etc.)
